...
def create_done_queue(index):
    """Queue used to signal death for i'th ps shard. Intended to have
    all workers enqueue an item onto it to signal doneness."""
    with tf.device("/job:ps/task:%d" % index):
        return tf.FIFOQueue(1, tf.int32, shared_name="done_queue" + str(index))
...
def main(argv=None):
    ...
    if FLAGS.job_name == 'ps':
        ...
        queue = create_done_queue(FLAGS.task_id)

        # wait until all workers are done
        for i in range(len(worker_hosts)):
            sess.run(queue.dequeue())
            print("ps %d received done %d" % (FLAGS.task_id, i))

        print("ps %d: quitting" % FLAGS.task_id)
    else:
        ...
        with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % FLAGS.task_id,
            ...
            with tf.train.MonitoredTrainingSession(master=server.target,
                ...
                while not mon_sess.should_stop() and step < FLAGS.max_number_of_steps:
